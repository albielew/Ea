...
def getDataSet(data, units, ignore_errors=False):
    ...
    try:
        ...
        if jsondata['type'] == 'rapid_wind':
            ...
            return EvtStrike(jsondata['evt'], units)
        elif jsondata['type'] == 'device_status':
            return DeviceStatus(jsondata, units)    
        elif jsondata['type'] == 'radio_stats':
            return DeviceStatus(jsondata, units)             
        else:
            return None
    except:
...
class EvtStrike:
    """Return the Lightning Strike Event data Structure."""
    def __init__(self, data, units):
        #Strike Event data
        self.type = 'evt_strike'
        self.strikeEpoch = data[0]
        self.strikeDistance = UnitConversion.distance(self, data[1], units)

class DeviceStatus:
    """Return the Device Status data Structure."""
    def __init__(self, data, units):
        #Device Status data
        self.type = 'device_status'
        self.timestamp = data['timestamp']
        self.sensor_status = data['sensor_status']
        self.radio = data['radio_stats']
